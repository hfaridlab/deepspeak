...
def train_model(
    dir_path, mod, num_epochs, pretrained_model_filename, test_model, batch_size, out_weight_name
):
    print("Loading data...")
...
def gen_parser():
    ...
    parser.add_option("-t", "--test", dest="test", action="store_true", help="run test on test dataset.")
    parser.add_option("-b", "--batch_size", dest="batch_size", type=int, help="batch size.")
    parser.add_option(
        "--out_weight_name",
        dest="out_weight_name",
        type=str,
        default=None,
        help=""
    )
    ...
    return dir_path, mod, epoch, pretrained_model_filename, test_model, int(batch_size), options.out_weight_name


def main():
    start_time = perf_counter()
    path, mod, epoch, pretrained_model_filename, test_model, batch_size, out_weight_name = gen_parser()
    train_model(path, mod, epoch, pretrained_model_filename, test_model, batch_size, out_weight_name)

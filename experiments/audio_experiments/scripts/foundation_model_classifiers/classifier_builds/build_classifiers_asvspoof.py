...
def load_embeddings(embedding_files):
    ...
    v1_1_files = [f for f in embedding_files]
...
def balance_data(X, y, random_state=42):
    ...
    print(f'Balanced data: {X_balanced.shape} {y_balanced.shape}')
...
def calculate_metrics(y_true, y_pred, y_prob):
    ...
    return {
        ...
        'Precision': precision,
        'Recall': recall,
        'Class Accuracy Real': class_accuracies[0],
        'Class Accuracy Fake': class_accuracies[1],
        'True Negatives': tn,
        'False Positives': fp,
        'False Negatives': fn,
        'True Positives': tp
...
def main():
    ...
    paths = config['paths']['classifiers']['asvspoof']
    ...
    for embedding in embedding_types:
        ...
        v1_1_data = load_embeddings(embedding_files)
        
        # Prepare input data
        print(f'{embedding}: Ingesting {dataset}data')
        ...
        for model, model_name in models:
            ...
            (X_v1_1_train, y_v1_1_train, X_v1_1_test, y_v1_1_test, 'v1_1')]:
                ...
                #Save model
                model_filename = f"{embedding}_{dataset}_{model_name.replace(' ', '_')}.joblib"

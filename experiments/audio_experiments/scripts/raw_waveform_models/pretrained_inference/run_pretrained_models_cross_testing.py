...
def load_audio(file_path, nb_samp):
    """Load audio in compatible format for pretrained models."""
...
def evaluate_model_on_dataset(model, device, config, dataset_path, model_name):
    ...
    # optional: balance dataset - have removed as we do not balance test set in the embeddings experiments 
    #min_count = min(len(real_df), len(fake_df))
    #real_df = real_df.sample(min_count, random_state=4)
    #fake_df = fake_df.sample(min_count, random_state=4)
...
def main():
    ...
    paths = config['paths']['pretrained_cross_testing']
